...
| 페르소나 ID | 이름 | 역할 | 대상 대시보드 | 우선순위 |
|------------|------|------|-------------|---------|
| P-01 | ML 엔지니어 / 데이터 분석가 | 실험 설계·실행·분석 담당 | 모델 탐색 | **Primary** |
| P-02 | MLOps 엔지니어 | 배포·운영 환경 구성 담당 | 양쪽 | Secondary |
| P-03 | 현장 검사 작업자 | 부품 검사 수행, 불량 조치 | 비전검사 | **Primary** |
| P-04 | 품질 관리자 | 불량률 모니터링, 이력 분석 | 비전검사 | Secondary |

> P-01이 모델 탐색 대시보드의 기능 결정 주 기준이다.
> P-03이 비전검사 대시보드의 기능 결정 주 기준이다.
...
| UC-16 | 불량 감지 팝업 확인 및 검사 재개 | P-03 | 검사탭1 | Must |
| UC-17 | 검사 이력 조회 및 CSV 내보내기 | P-04 | 검사탭2 | Must |
| UC-18 | 모델 교체 후 새 검사 세션 시작 | P-04 | 검사탭3, 검사탭1 | Should |
...
| **사용 환경** | 현장 PC 브라우저, 마우스·키보드 기본 조작 |
...
| # | 페인 포인트 | 이 제품의 해결 방식 |
|---|------------|-------------------|
| PP-10 | 불량이 발생해도 인지하지 못하고 라인이 계속 진행됨 | 불량 감지 즉시 팝업 알림 + 자동 검사 중지 |
| PP-11 | 판정 결과가 어디에 불량이 있는지 설명 없음 | Anomaly Map으로 불량 위치 시각적 표시 |
| PP-12 | 다음 부품 검사를 위해 복잡한 조작이 필요함 | 버튼 2개(수동/자동)로 단순화 |
...
```
액터:      P-03 (또는 P-04)
전제조건:  비전검사 대시보드 진입, history.json에 completed 실험 ≥ 1개
성공 종료: insp_active_model 설정 완료, 탭1에서 검사 가능 상태

주 플로우:
  1. 사이드바에서 [🏭 비전검사 대시보드] 클릭
  2. 탭3 [딥러닝 모델 교체] 진입
  3. 완료 실험 목록 테이블 확인 (F1 내림차순 기본 정렬)
  4. 원하는 실험의 [적용] 버튼 클릭
  5. 확인 다이얼로그: [교체 확인] 클릭
  6. insp_active_model 갱신 + reset_inspection_state() 실행
  7. 탭1 [실시간 검사]로 자동 이동
```
...
```
액터:      P-03
전제조건:  UC-13 완료, insp_active_model != None
성공 종료: inspection_record 1개 추가, 탭1에 판정결과/원본/Anomaly Map 표시

주 플로우:
  1. 탭1 [실시간 검사] 진입 (적용 모델명 상단 확인)
  2. [수동 검사 (1개 검사)] 클릭
  3. 시스템이 test_pool에서 이미지 1장 샘플링 → 추론 실행
  4. 3열 결과 표시: 판정카드(양품/불량 색상) / 원본 이미지 / Anomaly Map 히트맵
  5. 하단 미니 이력 바 갱신 (최근 10건)
  6. 양품이면 버튼 활성 유지, 다음 검사 대기
  7. 불량이면 UC-16 팝업 플로우 진행

비고:
  - 새 결과가 나오기 전까지 직전 결과가 화면에 유지됨 (R-INSP-06)
```
...
주 플로우:
  1. [자동 검사 (3초마다 1개)] 클릭 → insp_auto_active = True → st.rerun()
  2. 버튼이 [자동 검사 중지]로 변경됨
  3. 3초마다 자동으로 추론 실행 (UC-14 3~5 반복)
  4. [자동 검사 중지] 클릭 → insp_auto_active = False → 루프 종료

대안 플로우:
  3-1. 불량 감지 → insp_auto_active = False + insp_defect_popup = True → UC-16 진행
```
...
```
액터:      P-03
전제조건:  추론 결과 verdict == "불량"
성공 종료: 팝업 닫힘, 검사 재개 가능 상태

주 플로우:
  1. 불량 판정 즉시 st.dialog 팝업 표시
     "⚠️ 불량이 감지되었습니다. 해당 부품을 라인에서 제거하고 확인해 주세요."
     "이미지: {image_name} | Score: {score} (임계값: {threshold})"
     [ 확인 ] 버튼
  2. 자동 검사 중이었다면 이미 중지된 상태 (UC-15 대안 플로우)
  3. [확인] 클릭 → insp_defect_popup = False → 팝업 닫힘
  4. 수동/자동 버튼을 다시 눌러야 검사 재개 (자동 재개 없음)
```
...
액터:      P-04
전제조건:  검사 이력 ≥ 1건
성공 종료: 이력 테이블 확인, CSV 파일 다운로드

주 플로우:
  1. 탭2 [검사 이력 및 통계] 진입
  2. 이력 테이블 확인 (번호/시각/이미지명/판정결과/Anomaly Score, 역순)
  3. 필터 적용 (전체 / 양품만 / 불량만)
  4. KPI 카드 확인 (총 검사/양품/불량/불량률)
  5. [CSV 내보내기] 클릭 → 5컬럼 CSV 다운로드

대안 플로우:
  5-1. [이력 초기화] 클릭 → 확인 다이얼로그 → [확인] → reset_inspection_state()
```
...
```
액터:      P-04
전제조건:  비전검사 대시보드 진입, 더 좋은 완료 실험 존재
성공 종료: 새 모델 적용, 기존 이력 초기화, 새 검사 세션 시작

주 플로우:
  1. 탭3 [딥러닝 모델 교체] 진입
  2. 현재 적용 모델 정보 카드 확인
  3. 다른 실험의 [적용] 클릭
  4. 확인 다이얼로그: "현재 세션의 검사 이력이 모두 초기화됩니다. [취소] [교체 확인]"
  5. [교체 확인] → insp_active_model 갱신 + reset_inspection_state()
  6. 탭1으로 이동 → 새 모델로 검사 시작 대기
```
