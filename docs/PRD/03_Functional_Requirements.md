...
- [B. Detailed Specification](#b-detailed-specification)
  ...
  - [B.7 비전검사 대시보드 (INSP) — v1.1 추가](#b7-비전검사-대시보드-insp--v11-추가)
...
| **[학습 중지] 동작** | 1. `stop_event.set()` + `pause_event.clear()` (일시정지 중이면 해제 후 스레드가 stop_event 감지) <br> 2. 백그라운드 스레드 종료 <br> 3. FR-T3-06 중단 처리 실행 |
...
- [ ] FR-T3-09: 일시정지 후 ./models/checkpoints/{exp_id}_step{N}.ckpt 파일 존재
- [ ] FR-T3-09: 일시정지 상태에서 자동 rerun 중단 및 체크포인트 경로 표시
- [ ] FR-T3-10: 체크포인트 목록 정상 표시 (idle 상태)
- [ ] FR-T3-10: 체크포인트에서 재시작 시 중단 지점부터 학습 재개
