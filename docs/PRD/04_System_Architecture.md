...
| ADR-05 | 전처리 파이프라인은 `utils/image_utils.py` 단일 구현 | 탭2 미리보기와 탭3 학습 루프가 동일 코드 사용, 불일치 방지 |
...
│  │   render_sidebar()      │  st.tabs([탭1~탭5])               │    │
│  └─────────────────────────────────────────────────────────────┘    │
│                             │                                        │
│  ┌──────────┬──────────▼──────────┬──────────┬──────────┬───────┐  │
│  │  tab1    │  tab2_config.py     │  tab3    │tab4   │(tab5│  │
│  │_data_    │  탭2: 전처리 및     │ _training.py  │ _history.py  │_anomaly_map.py)│  │
│  │folder.py │  모델 설정 통합     │ 탭3 학습 │탭4 히스  │탭5시각│  │
│  └────┬─────┴──────────┬──────────┴────┬─────┴────┬─────┴───┬───┘  │
│       │                │               │          │         │       │
│  ┌────▼──────────▼──────────▼──────────▼──────────▼─────────▼────┐  │
│  │                       utils/ 레이어                             │  │
...
| `tab2_config.py` (탭2 통합) | `render()` | 전처리 영역·모델 영역 UI, 미리보기, preprocessing_config + model_config 동시 저장 |
| `tab3_training.py` (탭3) | `render()` | 실험명 입력, 학습 시작/일시정지/재시작/중지, Progress Bar, Loss 곡선, 로그, 체크포인트 재시작 섹션 |
| `tab4_history.py` (탭4) | `render()` | 히스토리 테이블, 상세 결과 차트, 비교 차트, 모델 저장, 삭제 |
| `tab5_anomaly_map.py` (탭5) | `render()` | 이미지 목록 테이블, Threshold 슬라이더, 3분할 시각화, PNG 저장 |
...
app.py
  ...
  │     └── utils/messages.py
  ├── tabs/tab2_config.py [탭2: 전처리 및 모델 설정]
  │     └── utils/image_utils.py (미리보기 전처리)
  │     └── utils/config_manager.py
  │     └── utils/messages.py
  ├── tabs/tab3_training.py [탭3: 학습]
  │     └── utils/model_factory.py
  │     │     └── utils/training_worker.py
  │     │           └── utils/image_utils.py
  │     │           └── (Anomalib, PyTorch)
  ...
  │     └── utils/config_manager.py
  │     └── utils/messages.py
  ├── tabs/tab4_history.py [탭4: 실험 히스토리]
  │     └── utils/config_manager.py (history.json 로드)
  │     └── utils/metrics.py (ROC 계산)
  │     └── utils/messages.py
  └── tabs/tab5_anomaly_map.py [탭5: 이상 영역 시각화]
...
접두사 `_`가 붙은 키는 내부 상태 키이며, 탭 간 공유 불필요. 탭3 내부에서만 사용한다.
...
```
사용자          app.py          tab3_training.py [탭3]   TrainingWorker      파일시스템
  │                │                    │                      │               │
  │ [학습 시작]    │                    │                      │               │
  ├──────────────►│                    │                      │               │
  │               │  탭3.render()      │                      │               │
...
#### B.8.2 탭5 Anomaly Map 시퀀스

```
사용자      tab5_anomaly_map.py [탭5]   model_factory.py    image_utils.py    파일시스템
  │                 │                    │                   │               │
  │ [이미지 선택]   │                    │                   │               │
  ├────────────────►│                    │                   │               │
...
| `config_manager.load_config()` | `yaml.YAMLError` | tab2 → `st.error(ERR_CONFIG_LOAD_FAILED)` | UI 상태 유지 |
| `TrainingWorker.run()` — GPU OOM | `torch.cuda.OutOfMemoryError` | queue → tab3 → `st.error()` | status="idle" |
| `TrainingWorker.run()` — 일반 예외 | `Exception` | queue(type="error") → tab3 → `st.error()` | status="idle" |
| `model_factory.load_model_for_inference()` | `FileNotFoundError` | tab5 → `st.error()` | 시각화 미렌더링 |
| `save_history()` — 디스크 풀 | `OSError` | tab3 → `st.error(ERR_MODEL_SAVE_FAILED)` | session_state 유지 |
...
|----|------|--------|----------|---------|
| `dataset_path` | `str\|None` | None | 탭1 | 탭2, 탭3 |
| `dataset_meta` | `dict\|None` | None | 탭1 | 탭2, 탭5 |
| `preprocessing_config` | `dict\|None` | None | 탭2 | 탭3 |
| `model_config` | `dict\|None` | None | 탭2 | 탭3 |
| `device_info` | `dict\|None` | None | 탭2 | 탭3, sidebar |
| `experiments` | `dict` | `{}` | 탭3, 탭4 | 탭4, 탭5 |
| `current_run_status` | `str` | `"idle"` | 탭3 | 탭3 |
| `current_exp_id` | `str\|None` | None | 탭3 | 탭3 |
| `selected_experiment_id` | `str\|None` | None | 탭4 | 탭5 |
| `anomaly_map_threshold` | `float\|None` | None | 탭5 | 탭5 |
| `_stop_event` | `Event\|None` | None | 탭3 내부 | 탭3 내부 |
| `_result_queue` | `Queue\|None` | None | 탭3 내부 | 탭3 내부 |
| `_progress` | `dict\|None` | None | 탭3 내부 | 탭3 내부 |
| `_log_lines` | `list` | `[]` | 탭3 내부 | 탭3 내부 |
| `_loss_history` | `list` | `[]` | 탭3 내부 | 탭3 내부 |
...
Then:   session_state가 초기화된다 (current_run_status = "idle")
        ...
        새로운 세션에서 탭3는 "idle" 상태로 렌더링
        history.json에는 중단된 실험 레코드가 없음 (새로고침 시 handle_stopped 미실행)
        → 탭3에 "학습 중 새로고침 시 학습 상태를 확인할 수 없습니다." 안내 텍스트 상시 표시
```
...
N/A — 전체 구현 계획은 14_Deployment_and_Release_Plan.md에서 다룬다.
      ...
      - 탭4 학습 비동기 구조(B.5절)를 모르고 tab3_training.py를 구현하면
        UI 블로킹 버그가 발생한다.
