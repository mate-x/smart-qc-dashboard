...
│                             │                                        │
│  ┌──────────┬──────────▼──────────┬──────────┬──────────┬───────┐  │
│  │  tab1    │  (파일명 미확정)     │(파일명   │(파일명   │(파일명│  │
│  │_data_    │  탭2: 전처리 및     │ 미확정)  │ 미확정)  │미확정)│  │
...
| `(파일명 미확정)` (탭2 통합) | `render()` | 전처리 영역·모델 영역 UI, 미리보기, preprocessing_config + model_config 동시 저장 |
| `(파일명 미확정)` (탭3) | `render()` | 실험명 입력, 학습 시작/중지, Progress Bar, Loss 곡선, 로그 |
| `(파일명 미확정)` (탭4) | `render()` | 히스토리 테이블, 상세 결과 차트, 비교 차트, 모델 저장, 삭제 |
| `(파일명 미확정)` (탭5) | `render()` | 이미지 목록 테이블, Threshold 슬라이더, 3분할 시각화, PNG 저장 |
...
```
사용자          app.py          (파일명 미확정) [탭3]   TrainingWorker      파일시스템
  │                │                    │                      │               │
  │ [학습 시작]    │                    │                      │               │
  ├──────────────►│                    │                      │               │
  │               │  탭3.render()      │                      │               │
...
```
사용자      (파일명 미확정) [탭5]   model_factory.py    image_utils.py    파일시스템
  │                 │                    │                   │               │
  │ [이미지 선택]   │                    │                   │               │
  ├────────────────►│                    │                   │               │
...
│   ├── __init__.py
│   ├── tab1_data_folder.py
│   ├── (통합 파일, 파일명 미확정)    # 탭2: 전처리 및 모델 설정
│   ├── (파일명 미확정)               # 탭3: 학습
│   ├── (파일명 미확정)               # 탭4: 실험 히스토리
│   └── (파일명 미확정)               # 탭5: 이상 영역 시각화
│
├── utils/
│   ├── __init__.py
