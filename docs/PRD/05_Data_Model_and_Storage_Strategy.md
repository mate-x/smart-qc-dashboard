...
### ADR-DS-01: RDBMS 미사용, 파일시스템 기반 영속화

| 항목 | 내용 |
|------|------|
| **결정** | SQLite를 포함한 모든 RDBMS를 사용하지 않는다. 영속 데이터는 JSON, YAML, `.pth` 파일로만 저장한다. |
| **근거** | MVP 단일 사용자 환경(A-01). 트랜잭션 격리 수준이 필요 없고, Docker 컨테이너 볼륨 마운트만으로 데이터 접근이 충분하다. |
