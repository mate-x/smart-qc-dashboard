# 09. Infrastructure and Environment

> **참조 기준**: [00_Global_Context_Document.md](./00_Global_Context_Document.md)
> **선행 문서**: [04_System_Architecture.md](./04_System_Architecture.md), [05_Data_Model_and_Storage_Strategy.md](./05_Data_Model_and_Storage_Strategy.md)
> **버전**: v2.0
> **작성일**: 2026-05-09
> **수정일**: 2026-06-11
> **중요**: 이 문서는 실행 환경의 Single Source of Truth다. 3개 레포 각각의 포트·빌드 명령·의존성, 하드웨어 요구사항, Python 환경, 디렉터리 초기화, 선택적 Docker 패키징 방법을 확정한다.

---

## 버전 히스토리

| 버전 | 날짜 | 변경 요약 |
|------|------|-----------|
| v1.0 | 2026-05-09 | 최초 작성 — Streamlit 단독 앱, 로컬 실행 기반 |
| v1.1 | 2026-05-26 | 비전검사 대시보드 인프라 추가 (J절 — Streamlit inspection/) |
| v2.0 | 2026-06-11 | 3개 레포 구조(Explorer/Vision/Dashboard) 반영; FastAPI :8000 공식화; React 앱 실행 환경(포트·빌드 명령·Node.js) 추가; Streamlit → 개발자 보조 도구로 격하; J절 smart-qc-vision 독립 레포로 교체 |

---

## 목차

- [A. Objective & Scope](#a-objective--scope)
- [B. Hardware Requirements](#b-hardware-requirements)
- [C. Software Requirements](#c-software-requirements)
- [D. Python Environment Setup](#d-python-environment-setup)
- [E. Directory Structure Initialization](#e-directory-structure-initialization)
- [F. Dataset Preparation](#f-dataset-preparation)
- [G. Application Launch](#g-application-launch)
- [H. Optional Docker Packaging](#h-optional-docker-packaging)
- [I. Environment Validation Checklist](#i-environment-validation-checklist)
- [J. smart-qc-vision 인프라](#j-smart-qc-vision-인프라-v20)

---

## A. Objective & Scope

### A.1 이 문서의 목적

이 프로젝트는 **3개 레포 기반** FastAPI + React 아키텍처다 (v2.0 확정, 00절 §11 ADR-01).  
로컬 실행 기반이며 클라우드 배포 없이 연구자·엔지니어의 단일 워크스테이션에서 실행된다.

| 레포 | 역할 | 기본 포트 | 실행 명령 |
|------|------|-----------|----------|
| `smart-qc-dashboard` | FastAPI 백엔드 + ML 레이어 | **:8000** | `uvicorn api.main:app --reload --port 8000` |
| `smart-qc-explorer` | React 모델 탐색 프론트엔드 | **:5173** | `npm run dev` |
| `smart-qc-vision` | React 비전검사 프론트엔드 | **:5173** | `npm run dev` |

> Explorer와 Vision은 기본 포트가 동일(`5173`)하므로 동시 실행 시 한 쪽 포트를 변경해야 한다 (G.4 참조).  
> Streamlit은 공식 UI가 아닌 **개발자 디버그용 보조 도구**로만 유지한다 (00절 ADR-01 v2.0).

이 문서는 다음을 확정한다:

1. 실행에 필요한 최소/권장 하드웨어 사양
2. OS·Python·Node.js·CUDA 버전 요구사항
3. Python 가상환경 구성 및 의존성 설치 순서 (FastAPI 백엔드)
4. Node.js 패키지 설치 (React 프론트엔드)
5. 첫 실행 전 필수 디렉터리 및 파일 초기화 절차
6. ImageNet penalty 데이터셋 배치 방법
7. 선택적 Docker 컨테이너 패키징 (재현성 목적)

### A.2 스코프 외 사항

| 항목 | 이유 |
|------|------|
| 클라우드 서버 배포 (AWS, GCP, Azure) | MVP 스코프 아님 |
| CI/CD 파이프라인 | 14_Deployment 문서에서 다룸 |
| 분산 학습 (multi-GPU, DDP) | 단일 GPU 환경 대상 |
| 데이터베이스 서버 | 파일시스템 기반 설계 |

---

## B. Hardware Requirements

### B.1 최소 사양 (CPU 전용)

| 항목 | 최소 | 비고 |
|------|------|------|
| CPU | 8코어 이상 | PatchCore 코어셋 연산 CPU-bound |
| RAM | 16 GB | 이미지 배치 로딩 + anomaly map 캐시 |
| 디스크 | 50 GB 이상 여유 | 모델 가중치 + 로그 + 결과 이미지 |
| OS | Windows 10+ / Ubuntu 20.04+ / macOS 12+ | |

CPU 전용 실행 시 EfficientAD 학습(`train_steps=70000` 기본값)은 수시간 소요될 수 있다.

### B.2 권장 사양 (GPU 사용)

| 항목 | 권장 | 비고 |
|------|------|------|
| GPU | NVIDIA RTX 3070 이상 (VRAM 8 GB 이상) | VRAM < 8 GB 시 batch_size 축소 필요 |
| CUDA | 12.9 | PyTorch 2.7.0 이상 필수 (C.3 참조) |
| RAM | 32 GB | GPU 학습 중 시스템 RAM 별도 사용 |
| 디스크 | SSD 권장, 100 GB 여유 | |

> Explorer / Vision React 앱 자체는 GPU가 필요 없다. GPU는 FastAPI 백엔드의 ML 학습·추론에만 사용된다.

### B.3 VRAM 요구사항 추정

| 모델 | image_size | batch_size | 추정 VRAM |
|------|-----------|-----------|----------|
| EfficientAD small | 256 | 1 | ~3 GB |
| EfficientAD medium | 256 | 1 | ~5 GB |
| PatchCore wide_resnet50_2 | 256 | 16 | ~6 GB |
| PatchCore wide_resnet50_2 | 512 | 16 | ~10 GB |

> VRAM 부족 시 `batch_size=1`로 설정하거나 `image_size`를 줄인다.  
> 추론 시에는 학습 모델을 메모리에 로드하므로 학습 완료 후 `del model; torch.cuda.empty_cache()` 호출 (07절 §C.3).

---

## C. Software Requirements

### C.1 OS별 지원 상태

| OS | 지원 | 비고 |
|----|------|------|
| Windows 10/11 (x64) | ✅ 공식 지원 | PowerShell 7+ 권장 |
| Ubuntu 20.04 / 22.04 | ✅ 공식 지원 | |
| macOS 12+ (Apple Silicon) | ⚠️ 부분 지원 | GPU: MPS (Metal) / anomalib 일부 제한 가능 |
| macOS Intel | ⚠️ CPU 전용 | |

### C.2 Python 버전

```
Python 3.12 (확정 — 00절 §10.2 기준)
```

- Python 3.9 이하: `match-case` 구문 미지원 — 사용 금지
- Python 3.10 / 3.11: 호환 가능하나 공식 환경은 3.12

### C.3 CUDA / cuDNN 버전

| CUDA | cuDNN | PyTorch 버전 | 비고 |
|------|-------|-------------|------|
| **12.9** | **9.x** | ≥ 2.7.0 | **확정 — RTX 5060(sm_120/Blackwell) 최소 요구사항** |

NVIDIA 드라이버 버전: CUDA 12.9 기준 **577.xx 이상**.  
베이스 Docker 이미지: `nvcr.io/nvidia/cuda:12.8.1-runtime-ubuntu22.04` + `libcudnn9-cuda-12` (apt 설치).

> sm_120(Blackwell) GPU는 PyTorch 2.7.0 미만에서 "no kernel image" 오류 발생. `requirements.txt`의 `torch>=2.7.0` 제약이 이를 방지한다.

> GPU 없는 환경에서는 CUDA 설치 불필요. FastAPI가 `device == "cpu"` 자동 선택.

### C.4 Node.js 버전 (Explorer / Vision 필수)

```
Node.js 18 이상 (확정 — smart-qc-explorer README, smart-qc-vision README 기준)
npm 9 이상 (Node.js 18 기본 포함)
```

| 도구 | 최소 | 비고 |
|------|------|------|
| Node.js | 18 | `node --version` 확인 |
| npm | 9 | `npm --version` 확인 |

> React 프론트엔드(Explorer, Vision)는 Python 환경과 독립적으로 Node.js 환경이 필요하다.  
> FastAPI 백엔드(smart-qc-dashboard)에는 Node.js가 불필요하다.

---

## D. Python Environment Setup

### D.1 Conda 환경 (권장)

```bash
# 1. conda 환경 생성
conda create -n smart-qc python=3.12 -y
conda activate smart-qc

# 2. PyTorch 설치 (CUDA 12.8/12.9 기준 — sm_120(Blackwell) 지원)
conda install pytorch torchvision torchaudio pytorch-cuda=12.8 \
    -c pytorch -c nvidia -y

# CPU 전용:
# conda install pytorch torchvision torchaudio cpuonly -c pytorch -y

# 3. 나머지 의존성 설치
pip install -r requirements.txt
```

### D.2 venv 환경 (대안)

```bash
python -m venv .venv

# Windows
.venv\Scripts\activate

# Linux / macOS
source .venv/bin/activate

# PyTorch 설치 (CUDA 12.8/12.9 — sm_120(Blackwell) 지원)
pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu128

# 나머지 의존성
pip install -r requirements.txt
```

### D.3 requirements.txt 핵심 의존성

```
# requirements.txt (smart-qc-dashboard)

# Core ML
torch>=2.7.0
torchvision>=0.22.0
anomalib>=1.0.0

# API server (v2.0 — FastAPI 공식화)
fastapi>=0.111.0
uvicorn[standard]>=0.29.0
python-multipart>=0.0.9
websockets>=12.0

# Image processing
opencv-python>=4.8.0
Pillow>=10.0.0
numpy>=1.24.0

# Data / ML utilities
pandas>=2.0.0
plotly>=5.18.0
matplotlib>=3.7.0
scikit-learn>=1.3.0

# Utilities
PyYAML>=6.0.1
tqdm>=4.66.0

# Developer tools (optional — 개발자 디버그용 보조 UI)
# streamlit>=1.32.0
```

> `anomalib>=1.0.0` 설치 시 `EfficientAd`(소문자 d)와 `Patchcore`(소문자 c) 클래스명을 사용한다 (08절 §B.1 기준).  
> `streamlit`은 공식 UI가 아닌 개발자 보조 도구이므로 주석 처리한다. 디버그 목적으로만 필요 시 별도 설치한다.

### D.4 설치 검증

```python
# verify_env.py — 환경 설치 후 실행 (smart-qc-dashboard)
import torch
import anomalib
import fastapi
import cv2

print(f"Python:     {__import__('sys').version}")
print(f"PyTorch:    {torch.__version__}")
print(f"CUDA avail: {torch.cuda.is_available()}")
if torch.cuda.is_available():
    print(f"GPU:        {torch.cuda.get_device_name(0)}")
    print(f"VRAM:       {torch.cuda.get_device_properties(0).total_memory / 1e9:.1f} GB")
print(f"Anomalib:   {anomalib.__version__}")
print(f"FastAPI:    {fastapi.__version__}")
print(f"OpenCV:     {cv2.__version__}")
```

기대 출력 (GPU 환경 예시):
```
Python:     3.12.x (...)
PyTorch:    2.7.x+cu129
CUDA avail: True
GPU:        NVIDIA GeForce RTX 5060
VRAM:       16.0 GB
Anomalib:   1.x.x
FastAPI:    0.111.x
OpenCV:     4.8.x
```

### D.5 Node.js 의존성 설치 (React 프론트엔드)

```bash
# Explorer 의존성 설치
cd smart-qc-explorer
npm install

# Vision 의존성 설치
cd smart-qc-vision
npm install
```

주요 패키지 버전 (package.json 기준):

| 패키지 | Explorer | Vision |
|--------|----------|--------|
| React | 19 | 최신 |
| Vite | 8 | 최신 |
| TypeScript | ✅ | ✅ |
| Zustand | v5 | 최신 |
| react-router-dom | v7 | v6 |
| Recharts | v2 | ✅ |
| Tailwind CSS | v4 | — |
| Axios | ✅ | ✅ |

---

## E. Directory Structure Initialization

### E.1 필수 디렉터리 (smart-qc-dashboard)

최초 실행 전 아래 디렉터리가 존재해야 한다. 없으면 FastAPI 서버 시작 시 자동 생성한다.

```
smart-qc-dashboard/
├── experiments/            # history.json 저장 위치 (05절 §2.1)
├── models/                 # {exp_id}/ 하위 디렉터리 (05절 §2.2)
├── logs/                   # {exp_id}.log 저장 위치 (05절 §2.3)
├── results/                # {exp_id}/{stem}_triplet.png (07절 §C.4)
└── dataset/
    └── imagenet_penalty/   # EfficientAD ImageNet penalty 이미지 (05절 §2.4)
```

### E.2 자동 초기화 코드

```python
# utils/env_init.py — FastAPI 서버 시작 시 1회 호출
from pathlib import Path

REQUIRED_DIRS = [
    Path("./experiments"),
    Path("./models"),
    Path("./logs"),
    Path("./results"),
    Path("./dataset/imagenet_penalty"),
]

def ensure_required_dirs() -> None:
    for d in REQUIRED_DIRS:
        d.mkdir(parents=True, exist_ok=True)
```

```python
# api/main.py 상단 (FastAPI 앱 시작 시)
from utils.env_init import ensure_required_dirs
ensure_required_dirs()
```

### E.3 experiments/history.json 초기화

`history.json`이 존재하지 않을 경우 빈 배열로 초기화한다:

```python
# utils/storage.py — load_history() 내부 (05절 §3.1 참조)
HISTORY_FILE = Path("./experiments/history.json")

def load_history() -> list[dict]:
    if not HISTORY_FILE.exists():
        return []
    with open(HISTORY_FILE, "r", encoding="utf-8") as f:
        return json.load(f)
```

### E.4 3개 레포 디렉터리 구조 전체

```
c:\project01\
├── smart-qc-dashboard/          # FastAPI 백엔드 + ML 레이어
│   ├── api/                     # FastAPI 라우터
│   ├── utils/                   # storage, config_manager, model_factory 등
│   ├── experiments/             # history.json (런타임 생성)
│   ├── models/                  # 모델 가중치 (런타임 생성)
│   ├── logs/                    # 학습 로그 (런타임 생성)
│   ├── results/                 # Triplet 이미지 (런타임 생성)
│   └── dataset/
│       └── imagenet_penalty/    # EfficientAD용 (수동 배치)
│
├── smart-qc-explorer/           # React 모델 탐색 프론트엔드
│   ├── src/
│   ├── node_modules/            # npm install 후 생성
│   └── dist/                    # npm run build 후 생성
│
└── smart-qc-vision/             # React 비전검사 프론트엔드
    ├── src/
    ├── node_modules/            # npm install 후 생성
    └── dist/                    # npm run build 후 생성
```

---

## F. Dataset Preparation

### F.1 MVTec AD 형식 데이터셋 구조

이 시스템은 MVTec AD 형식 디렉터리 구조를 요구한다 (01절 §B.2 기준):

```
{dataset_root}/
├── train/
│   └── good/
│       ├── 000.png
│       └── ...
└── test/
    ├── good/
    │   ├── 000.png
    │   └── ...
    └── {defect_class}/
        ├── 000.png
        └── ...
```

Ground truth 마스크를 포함하는 경우 (선택적):

```
{dataset_root}/
└── ground_truth/
    └── {defect_class}/
        ├── 000_mask.png    # {stem}_mask{ext} 형식 (07절 §C.3 확정)
        └── ...
```

### F.2 지원 이미지 포맷

| 포맷 | 확장자 | 비고 |
|------|-------|------|
| PNG | `.png` | 권장 (무손실) |
| JPEG | `.jpg`, `.jpeg` | 허용 |
| BMP | `.bmp` | 허용 |
| TIFF | `.tiff`, `.tif` | 허용 |

지원 포맷 외 파일이 발견되면 `dataset_meta.has_invalid_files = True`로 표시하고 해당 파일을 학습에서 제외한다.

### F.3 ImageNet Penalty 데이터셋 (EfficientAD 전용)

EfficientAD 학습 시 `./dataset/imagenet_penalty/` 에 ImageNet 이미지가 필요하다 (05절 §2.4, 08절 Z.1).

```
dataset/
└── imagenet_penalty/
    ├── n01440764/          # ImageNet synset 디렉터리 (권장 구조)
    │   ├── img_0001.JPEG
    │   └── ...
    └── ...
```

**최소 이미지 수**: 1,000장 이상 권장 (학습 중 penalty 샘플링에 사용).

**경로 검증**: `validate_imagenet_penalty_dir()`가 아래 조건을 검사한다 (05절 §2.4):
- 디렉터리 존재 여부
- 내부 이미지 파일 1개 이상 존재 여부

조건 미충족 시 `(False, 0)`을 반환하며, Explorer에서 EfficientAD 학습 시작을 차단한다.

```bash
# imagenet_penalty 디렉터리 최소 검증 (bash)
find ./dataset/imagenet_penalty -type f \( -name "*.JPEG" -o -name "*.jpg" -o -name "*.png" \) | wc -l
# 출력이 0이면 EfficientAD 실행 불가
```

---

## G. Application Launch

### G.1 서비스 실행 순서

**FastAPI 백엔드가 먼저 실행되어야 한다.** Explorer / Vision은 백엔드 없이 동작하지 않는다.

```
1. Terminal 1: FastAPI 백엔드 (smart-qc-dashboard) → :8000
2. Terminal 2: Explorer 프론트엔드 (smart-qc-explorer) → :5173
3. Terminal 3: Vision 프론트엔드 (smart-qc-vision) → :5173 (또는 :5174)
```

> Explorer와 Vision은 기본 포트가 동일(`5173`)하므로 동시 실행 시 한 쪽을 다른 포트로 변경해야 한다.

### G.2 Terminal 1: FastAPI 백엔드 (smart-qc-dashboard)

```bash
# smart-qc-dashboard 루트에서 실행
conda activate smart-qc   # 또는 source .venv/bin/activate

uvicorn api.main:app --reload --port 8000
```

접속 확인: `http://localhost:8000/docs` (Swagger UI 자동 생성)

포트 변경 시:
```bash
uvicorn api.main:app --reload --port 8001
```

### G.3 Terminal 2: Explorer 프론트엔드 (smart-qc-explorer)

```bash
# smart-qc-explorer 루트에서 실행
npm run dev
```

브라우저에서 `http://localhost:5173` 접속.

백엔드 URL 기본값: `http://localhost:8000` (`src/api/client.ts` 기준).

### G.4 Terminal 3: Vision 프론트엔드 (smart-qc-vision)

```bash
# smart-qc-vision 루트에서 실행
npm run dev
```

브라우저에서 `http://localhost:5173` 접속.

**Explorer와 동시 실행 시 포트 충돌 대처:**
```bash
# 방법 1: Vite 직접 지정
npx vite --port 5174

# 방법 2: 환경변수 지정
VITE_PORT=5174 npm run dev
```

### G.5 프로덕션 빌드

```bash
# Explorer 빌드
cd smart-qc-explorer
npm run build  # → dist/ 폴더 생성

# Vision 빌드
cd smart-qc-vision
npm run build  # → dist/ 폴더 생성
```

빌드 결과물(`dist/`)은 nginx 또는 정적 파일 서버로 서빙할 수 있다.

### G.6 환경변수 설정

**smart-qc-dashboard** — `.env` (FastAPI):
```bash
# 필요 시 아래 항목 설정 (기본값으로 동작)
DEVICE=cuda        # 또는 cpu
LOG_LEVEL=info
```

**smart-qc-explorer** — `.env` (React):
```bash
# Vite 환경변수 (VITE_ 접두사 필수)
VITE_API_BASE_URL=http://localhost:8000
```

**smart-qc-vision** — `.env` (React):
```bash
VITE_API_BASE_URL=http://localhost:8000
```

> 백엔드 주소가 기본값(`localhost:8000`)과 다른 경우 각 프론트엔드의 `.env` 파일을 수정한다.

### G.7 실행 전 체크리스트

```
□ conda activate smart-qc (또는 .venv/bin/activate)
□ python verify_env.py — 모든 항목 정상 출력 확인
□ node --version — v18 이상 확인
□ npm install 완료 (smart-qc-explorer, smart-qc-vision 각각)
□ ./dataset/imagenet_penalty/ — EfficientAD 사용 시 이미지 존재 확인
□ 데이터셋 디렉터리 MVTec AD 형식 확인 (train/good/, test/{class}/)
□ uvicorn api.main:app --reload --port 8000 → http://localhost:8000/docs 접속 확인
□ npm run dev → http://localhost:5173 접속 확인
```

---

## H. Optional Docker Packaging

> Docker는 필수가 아닌 선택적 재현성 수단이다. 개발·실험 환경에서는 직접 실행을 권장한다.

### H.1 Dockerfile (smart-qc-dashboard — FastAPI 백엔드)

```dockerfile
FROM nvcr.io/nvidia/cuda:12.8.1-runtime-ubuntu22.04

WORKDIR /app

ENV DEBIAN_FRONTEND=noninteractive
ENV TZ=Asia/Seoul

# Python 3.12 및 시스템 의존성 설치
RUN apt-get update && apt-get install -y software-properties-common \
    && add-apt-repository ppa:deadsnakes/ppa \
    && apt-get update && apt-get install -y \
       python3.12 python3.12-venv python3.12-dev curl \
       libcudnn9-cuda-12 libgl1 \
    && curl -sS https://bootstrap.pypa.io/get-pip.py | python3.12 \
    && rm -rf /var/lib/apt/lists/*

COPY requirements.txt .

# 가상환경 생성 및 의존성 설치
RUN python3.12 -m venv /opt/venv
ENV PATH="/opt/venv/bin:$PATH"
RUN pip install --no-cache-dir opencv-python-headless && \
    pip install --no-cache-dir -r requirements.txt

# 소스 복사
COPY . .

# 필수 디렉터리 생성
RUN mkdir -p experiments models logs results dataset/imagenet_penalty

# FastAPI 포트
EXPOSE 8000

# 실행
ENTRYPOINT ["uvicorn", "api.main:app", \
            "--host", "0.0.0.0", \
            "--port", "8000"]
```

### H.2 docker-compose 구성 (base + override 구조)

공통 설정은 `docker-compose.base.yml`, 환경별 차이만 오버라이드 파일로 분리.

**docker-compose.base.yml** (공통):

```yaml
services:
  db:
    image: mysql:8.0
    container_name: smart-qc-db
    restart: unless-stopped
    environment:
      MYSQL_ROOT_PASSWORD: ${MYSQL_ROOT_PASSWORD:-1234}
      MYSQL_DATABASE: ${MYSQL_DATABASE:-smart-qc}
    ports:
      - "3307:3306"
    volumes:
      - db_data:/var/lib/mysql
    healthcheck:
      test: ["CMD", "mysqladmin", "ping", "-h", "localhost", "-u", "root", "-p${MYSQL_ROOT_PASSWORD:-1234}"]
      interval: 10s
      timeout: 5s
      retries: 10

  backend:
    build:
      context: ./smart-qc-dashboard
    container_name: smart-qc-backend
    restart: unless-stopped
    env_file:
      - ./smart-qc-dashboard/.env
    depends_on:
      db:
        condition: service_healthy
    ports:
      - "8000:8000"
    volumes:
      # 데이터·모델·결과는 호스트와 마운트 — 컨테이너 재시작 후 데이터 유지
      - ./smart-qc-dashboard/dataset:/app/dataset
      - ./smart-qc-dashboard/experiments:/app/experiments
      - ./smart-qc-dashboard/models:/app/models
      - ./smart-qc-dashboard/logs:/app/logs
      - ./smart-qc-dashboard/results:/app/results
    environment:
      DATABASE_URL: mysql+pymysql://root:${MYSQL_ROOT_PASSWORD:-1234}@db:3306/${MYSQL_DATABASE:-smart-qc}

  explorer:
    # [확인 필요: smart-qc-explorer Docker 빌드 설정 미확정]
    # 로컬 개발 시: npm run dev → localhost:5173
    # 프로덕션 시: npm run build → dist/ → nginx 정적 서빙 권장
    image: node:18-alpine
    container_name: smart-qc-explorer
    working_dir: /app
    command: sh -c "npm install && npm run dev -- --host 0.0.0.0"
    ports:
      - "5173:5173"
    volumes:
      - ./smart-qc-explorer:/app
      - /app/node_modules
    environment:
      VITE_API_BASE_URL: http://backend:8000

  vision:
    # [확인 필요: smart-qc-vision Docker 빌드 설정 미확정]
    image: node:18-alpine
    container_name: smart-qc-vision
    working_dir: /app
    command: sh -c "npm install && npm run dev -- --host 0.0.0.0 --port 5174"
    ports:
      - "5174:5174"
    volumes:
      - ./smart-qc-vision:/app
      - /app/node_modules
    environment:
      VITE_API_BASE_URL: http://backend:8000

volumes:
  db_data:
```

**docker-compose.yml** (GPU 오버라이드):

```yaml
services:
  backend:
    deploy:
      resources:
        reservations:
          devices:
            - driver: nvidia
              count: 1
              capabilities: [gpu]
    environment:
      NVIDIA_VISIBLE_DEVICES: 0
```

### H.3 Docker 실행

```bash
# GPU 빌드 및 실행
docker compose -f docker-compose.base.yml -f docker-compose.yml build
docker compose -f docker-compose.base.yml -f docker-compose.yml up -d

# CPU 전용
docker compose -f docker-compose.base.yml -f docker-compose.cpu.yml up -d

# 백엔드만 실행 (프론트엔드는 로컬 npm run dev)
docker compose -f docker-compose.base.yml up backend db -d
```

### H.4 Docker 사용 시 제약

| 항목 | 내용 |
|------|------|
| 데이터셋 경로 | 컨테이너 내부 `/app/dataset/` 로 마운트 필요 |
| ImageNet penalty | 호스트 `./smart-qc-dashboard/dataset/imagenet_penalty/` → 컨테이너 마운트 |
| 백엔드 접속 (호스트) | `http://localhost:8000` |
| Explorer 접속 (호스트) | `http://localhost:5173` |
| Vision 접속 (호스트) | `http://localhost:5174` (Docker에서 포트 충돌 방지) |
| Windows Docker Desktop | WSL2 백엔드 + NVIDIA Container Toolkit 필요 |
| DB 접속 (호스트) | `localhost:3307` (컨테이너 간 통신은 `db:3306`) |
| DB 시작 순서 | `backend` 컨테이너는 DB healthcheck 통과 후 기동 (`depends_on: service_healthy`) |
| 프론트엔드 Docker | [확인 필요] node:18-alpine dev 서버 방식은 프로덕션 미권장 — nginx 정적 서빙 Dockerfile 별도 작성 권장 |

---

## I. Environment Validation Checklist

### I.1 설치 완료 기준

| 검증 항목 | 명령 | 기대 결과 |
|-----------|------|----------|
| Python 버전 | `python --version` | `3.12.x` |
| PyTorch 설치 | `python -c "import torch; print(torch.__version__)"` | `2.7.x+cu129` 등 |
| CUDA 사용 가능 | `python -c "import torch; print(torch.cuda.is_available())"` | `True` (GPU 환경) |
| Anomalib 설치 | `python -c "import anomalib; print(anomalib.__version__)"` | `1.x.x` |
| FastAPI 설치 | `python -c "import fastapi; print(fastapi.__version__)"` | `0.111.x+` |
| uvicorn 실행 | `uvicorn --version` | `0.29.x` 등 |
| Node.js 버전 | `node --version` | `v18.x.x` 이상 |
| npm 버전 | `npm --version` | `9.x.x` 이상 |
| 디렉터리 존재 | `ls experiments/ models/ logs/ results/` | 오류 없음 |
| FastAPI 서버 응답 | `curl http://localhost:8000/docs` | HTML 반환 (서버 실행 후) |

### I.2 EfficientAD 실행 가능 기준 추가 검증

```bash
# ImageNet penalty 이미지 존재 확인
python -c "
from utils.storage import validate_imagenet_penalty_dir
ok, count = validate_imagenet_penalty_dir()
if ok:
    print(f'OK: ImageNet penalty dir is valid ({count}장)')
else:
    print('FAIL: ImageNet penalty dir missing or empty')
"
```

### I.3 첫 실행 전 전체 검증 스크립트

```python
# scripts/preflight_check.py
import sys
import subprocess
from pathlib import Path

CHECKS = []

# Python 버전
major, minor = sys.version_info[:2]
CHECKS.append(("Python 3.12", (major, minor) == (3, 12)))

# PyTorch
try:
    import torch
    CHECKS.append(("PyTorch >= 2.7", tuple(int(x) for x in torch.__version__.split("+")[0].split(".")[:2]) >= (2, 7)))
    CHECKS.append(("CUDA available", torch.cuda.is_available()))
except ImportError:
    CHECKS.append(("PyTorch", False))

# Anomalib
try:
    import anomalib
    CHECKS.append(("Anomalib >= 1.0", anomalib.__version__ >= "1.0"))
except ImportError:
    CHECKS.append(("Anomalib", False))

# FastAPI
try:
    import fastapi
    CHECKS.append(("FastAPI >= 0.111", fastapi.__version__ >= "0.111"))
except ImportError:
    CHECKS.append(("FastAPI", False))

# uvicorn
try:
    import uvicorn
    CHECKS.append(("uvicorn installed", True))
except ImportError:
    CHECKS.append(("uvicorn", False))

# Required dirs
for d in ["experiments", "models", "logs", "results", "dataset/imagenet_penalty"]:
    CHECKS.append((f"Dir: {d}", Path(f"./{d}").exists()))

# Node.js version check
try:
    result = subprocess.run(["node", "--version"], capture_output=True, text=True, timeout=5)
    node_version = result.stdout.strip()  # e.g. "v18.17.0"
    major = int(node_version.lstrip("v").split(".")[0])
    CHECKS.append(("Node.js >= 18", major >= 18))
except Exception:
    CHECKS.append(("Node.js >= 18", False))

# Report
print("\n=== Preflight Check ===")
all_ok = True
for name, result in CHECKS:
    status = "✅" if result else "❌"
    print(f"  {status}  {name}")
    if not result:
        all_ok = False

print("\n" + ("All checks passed." if all_ok else "Some checks FAILED — fix before running."))
sys.exit(0 if all_ok else 1)
```

---

## J. smart-qc-vision 인프라 (v2.0)

> **v2.0 변경**: v1.1의 Streamlit `inspection/` 디렉터리 구조가 v2.0에서 독립 레포(`smart-qc-vision`)로 분리됐다.  
> `smart-qc-dashboard`의 `inspection/` 디렉터리는 더 이상 사용되지 않는다.

### J.1 smart-qc-vision 레포 개요

`smart-qc-vision`은 Vite + React + TypeScript 기반의 독립적인 프론트엔드 프로젝트다.

| 항목 | 내용 |
|------|------|
| 레포 위치 | `c:\project01\smart-qc-vision\` (별도 레포) |
| 기술 스택 | Vite + React + TypeScript + Zustand + Recharts |
| 라우팅 | react-router-dom v6 |
| 기본 포트 | `:5173` |
| 빌드 명령 | `npm run build` → `dist/` |
| 개발 서버 | `npm run dev` |
| 백엔드 의존성 | `smart-qc-dashboard` FastAPI `:8000` 필수 |

### J.2 화면 구성

| 경로 | 탭 | 기능 |
|------|----|------|
| `/` | 실시간 검사 | 수동/자동 비전 검사, 원본·Anomaly Map·오버레이 표시 |
| `/history` | 검사 이력 | KPI 카드, 이력 테이블, 통계 차트, CSV 내보내기 |
| `/models` | 모델 교체 | 학습 완료 모델 목록 조회 및 적용 |

### J.3 설치 및 실행

```bash
# smart-qc-vision 루트에서
cd smart-qc-vision

# 최초 1회: 의존성 설치
npm install

# 개발 서버 시작
npm run dev   # → http://localhost:5173

# Explorer와 동시 실행 시 (포트 충돌 방지)
npx vite --port 5174   # → http://localhost:5174

# 프로덕션 빌드
npm run build   # → dist/ 폴더
```

### J.4 백엔드 연동 API

Vision이 사용하는 FastAPI 엔드포인트:

| 메서드 | 엔드포인트 | 설명 |
|--------|-----------|------|
| GET | `/api/models` | 실험 목록 조회 (모델 교체 탭) |
| GET | `/api/inspection/model` | 현재 적용 모델 조회 |
| POST | `/api/inspection/model` | 모델 적용 |
| POST | `/api/inspection/run` | 수동 검사 1회 실행 |
| GET | `/api/inspection/records` | 검사 이력 조회 |
| DELETE | `/api/inspection/records` | 검사 이력 초기화 |
| GET | `/api/inspection/records/csv` | CSV 다운로드 |
| GET | `/api/inspection/image/last` | 마지막 검사 원본 이미지 |
| GET | `/api/inspection/anomaly-map/last` | Anomaly Map 이미지 |
| GET | `/api/inspection/overlay/last` | 오버레이 이미지 |
| WS | `/ws/inspection/auto` | 자동 검사 WebSocket |

### J.5 테스트 데이터 출처

Vision 탭1(실시간 검사)의 테스트 이미지는 `smart-qc-dashboard`의 `dataset/` 경로에서 샘플링한다. 해당 경로는 FastAPI 서버가 관리한다.

```
# FastAPI 서버 측 (smart-qc-dashboard)
dataset/
└── {category}/
    └── test/
        ├── good/
        └── {defect_class}/
```

### J.6 Docker 환경에서의 Vision 프론트엔드

| 항목 | 내용 |
|------|------|
| 개발 환경 | `npm run dev` 직접 실행 (H.2 `vision` 서비스) |
| 프로덕션 환경 | `npm run build` → `dist/` → nginx 정적 서빙 권장 |
| 백엔드 연결 | `VITE_API_BASE_URL=http://backend:8000` 환경변수 |
| 포트 (Docker) | `5174` 권장 (Explorer와 충돌 방지) |
| [확인 필요] | 프로덕션용 nginx Dockerfile 미확정 |

### J.7 I.1 검증 항목 추가

| 검증 항목 | 명령 | 기대 결과 |
|-----------|------|----------|
| smart-qc-vision npm 설치 | `ls smart-qc-vision/node_modules` | 오류 없음 |
| Vision 빌드 확인 | `cd smart-qc-vision && npm run build` | dist/ 생성, 오류 없음 |
| Vision API 연결 | Vision 앱에서 `/models` 탭 접속 | 실험 목록 로드 (FastAPI 실행 중이어야 함) |

---

*다음 문서*: [11_Non_Functional_Requirements.md](./11_Non_Functional_Requirements.md)
