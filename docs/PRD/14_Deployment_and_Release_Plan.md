...
```
□ 탭1: 데이터셋 경로 입력 → 스캔 성공
□ 탭2: 전처리 및 모델 설정 저장 (PatchCore, resnet18, train_steps 제한 포함) → 미리보기 표시
□ 탭3: 학습 시작 → Progress bar 갱신 → 완료 메시지
□ 탭4: 실험 목록 표시 → 선택 → 결과 상세 확인
□ 탭5: 추론 실행 → Anomaly Map 표시 → PNG 저장
□ [학습 중지] 버튼 동작 확인
```
