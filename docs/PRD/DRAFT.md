...
| **작성일** | 2026-05-08 |
| **수정일** | 2026-05-26 |
| **작성 목적** | 자동차 볼트 외관 결함 탐지를 위한 비전 검사 모델(EfficientAD, PatchCore)의 학습·평가·비교를 🔬 모델 탐색 대시보드에서, 학습된 모델을 활용한 실시간 비전검사를 🏭 비전검사 대시보드에서 수행할 수 있도록 기능·비기능 요구사항을 명세 (v1.1에서 이중 대시보드 구조로 확장) |
...
## 비전검사 대시보드 개요 (v1.1 추가)

### 이중 대시보드 구조

v1.1에서 시스템은 두 개의 대시보드로 확장된다:

| 대시보드 | 버튼 | 목적 |
|---------|------|------|
| 🔬 모델 탐색 | 사이드바 상단 | EfficientAD / PatchCore 학습·평가·비교 (기존 6탭) |
| 🏭 비전검사 | 사이드바 상단 | 학습된 모델을 이용한 실시간 비전검사 (신규 3탭) |

사이드바에는 데이터셋/디바이스 정보 없이 두 개의 전환 버튼만 표시된다.

### 비전검사 대시보드 탭 구성

- **탭1 (실시간 검사)**: 수동 검사 버튼 + 자동 검사(3초 간격) 버튼; 3열 레이아웃(판정결과 / 원본이미지 / Anomaly Map); 불량 감지 시 팝업 표시 및 자동 검사 중지
- **탭2 (검사 이력 및 통계)**: 5열 이력 테이블(번호/시각/이미지명/판정결과/Anomaly Score) + KPI 카드 4개(총검사/양품/불량/불량률); 히스토그램/차트 없음
- **탭3 (딥러닝 모델 교체)**: 완료된 실험 목록 F1 기준 정렬; 모델 교체 시 모든 검사 이력 초기화

### 핵심 설계 원칙

- `insp_` 네임스페이스로 모든 session_state 키 분리 (R-INSP-01)
- 세션 전용 저장소 — 파일시스템 기록 없음, 앱 재시작 또는 모델 교체 시 초기화
- 신규 파일: `inspection/` 디렉터리 (inspection_app.py, 탭 3개, 유틸 2개)
- NFR: 추론 ≤ 3초, 자동 타이밍 ±0.5초, 팝업 ≤ 0.5초
...
| v1.1 | 2026-05-26 | ML 엔지니어링팀 | 비전검사 대시보드(이중 대시보드 구조) 추가 |
